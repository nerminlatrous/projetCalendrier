...
<?php 
    ...
    $events=$events->getEventsBetwenByDay($start,$end);
                ...
                <td class=" <?= $month->withinMonth($date) ? '' :'calendar__othermonth' ; ?> <?= $isToday ? 'is_today' : '' ?> ">
                    ...
                    <?php foreach($eventsForDay as $event) : ?>
                       <div class="calendar__event">
                           ...
                           <a href="/event.php?id=<?=$event['id']; ?>" style="color:red !important;"> <?= $event['name']; ?>
                           <?php elseif ($event['status'] === 1) : ?>
                           <a href="/event.php?id=<?=$event['id']; ?>" style="color:green !important;"> <?= $event['name']; ?>
                           <?php elseif ($event['status'] === 0) : ?>
                           <a href="/event.php?id=<?=$event['id']; ?>" style="color:orange !important;"> <?= $event['name']; ?>
                           <?php endif ?>
                       </div>
                    <?php  endforeach ;?>
                </td>
